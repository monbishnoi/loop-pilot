...
The model treats this as context, not as a command. High-confidence guidance can be optimized around. Medium-confidence guidance is a planning prior. If confidence is low, Loop Pilot returns no injectable prompt guidance at all. If a task looks complex but the predicted budget is suspiciously small, the prompt says so explicitly.
